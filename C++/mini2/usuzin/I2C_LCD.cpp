...
void I2C_LCD::I2C_LCD_write_data(uint8_t data)
{
	char data_u, data_l;
	uint8_t data_t[4] = {0,};
	...
	for(char i=0;i<4;i++)
	{
		i2c.I2C_write_byte(SLA_W, data_t[i]);
	}
}

void I2C_LCD::I2C_LCD_write_command(uint8_t command)
{
	char data_u, data_l;
	uint8_t data_t[4];
...
void I2C_LCD::I2C_LCD_clear(void)
{
	I2C_LCD_write_command(COMMAND_CLEAR_DISPLAY);
	_delay_ms(2);
}

void I2C_LCD::I2C_LCD_init(void)
{
	i2c.I2C_init(10000);
	_delay_ms(50);
...
void I2C_LCD::I2C_LCD_write_string(char *string)
{
	uint8_t i;
	for(i=0; string[i]; i++)
...
void I2C_LCD::I2C_LCD_goto_XY(uint8_t row, uint8_t col)
{
	col %= 16;
	row %= 2;
...
void I2C_LCD::I2C_LCD_write_string_XY(uint8_t row, uint8_t col, char *string)
{
